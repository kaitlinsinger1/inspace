...
list.of.packages <- c("shiny","ggmap", "excelR", 'rhandsontable', 'DT', 'tidycensus', 'tidyverse', 'dplyr', 'janitor', 'reshape2', 
                      'promises', 'future', 'shinythemes', 'shinyjs')
           ...
           navlistPanel(widths=c(3, 9),
                        ...
                        tabPanel('Upload Dataset',
                                 ...
                                 fluidRow(style = 'padding-left:30px',
                                          em('**NOTE: If you have previously uploaded, geocoded, and/or pulled environmental measures with this tool, this will overwrite prior data unless you rename your previous datasets'),
                          ...
                          fluidRow(style='padding-left:30px',
                                   column(width=4,
                                          ...
                                          tags$ul(
                                            ...
                                            tags$li("Note that your data will be updated and saved in your R environment with each update")
                                          ),
                          ...
                          conditionalPanel(
                            ...
                            fluidRow(
                              column(width=4,
                                     ...
                                     wellPanel(
                                       actionButton('Geocodecomplete', 'Geocode Checking Complete')
                                     )
                              ), 
                              column(width=8,
...
observeEvent(input$dataset_geocoded2_cell_edit, {
  dataset_geocoded<-loadData('dataset_geocoded.csv')%>%mutate(address=as.character(address), geocode_notes=as.character(geocode_notes))%>%filter(!is.na(id))
  filetable$data<-dataset_geocoded
...
observe({
  ...
  isolate({
    ...
    updateSelectInput(session, 'idNumber', 
                      ...
                      choices=c('Select a single id to map', filter_uploaded_data()$id), 
...
##refresh page button (after geocode checking is complete)
observeEvent(input$Geocodingcomplete, {
  session$reload()
})
...
load_geocode_acs<-eventReactive(input$loaddata_acs,{
  source('~/workspace/Inspace/data_pull_settings/acs_data_settings.R')
  loadData('dataset_geocoded.csv')%>%dplyr::select(id, lat, long)%>%filter(!is.na(lat))
})
...
observeEvent(input$pull_county_GEOID, {
  ...
  auto_status(id='all ids', radius='NA', year='2010')
  fire_running()
  
  #loop to pull county GEOIDs: 
  dataset_geocoded<-load_geocode_acs()
  withProgress(
    dataset_GEOID_county<-pull_county_geoid(dataset_geocoded),  message='Pulling County GEOIDs')
  write.csv(dataset_GEOID_county, '~/workspace/Inspace/data_pull_measures/dataset_county_geoid.csv')
  nclicks(0)
  
  # Notify status file of progress
  result_val(data.frame(Status="County GEOID pull complete"))
...
observeEvent(input$pull_rpp,{
  ...
  withProgress(pull_rpp_measures(dataset_geocoded), message='Processing RPP data')
  ...
  for(y in years){
       ...
       # withProgress(message='Pulling RPP data', {
        ...
        auto_status(id='pulling all ids', radius='NA', year=y)
...
observeEvent(input$show_data_rpp, {
  ...
  if(input$show_data_rpp=='Show measure summary'& file.exists('~/workspace/Inspace/data_pull_measures/dataset_rpp.csv')==TRUE){
    preview_rpp$data=read.csv('~/workspace/Inspace/data_pull_measures/dataset_rpp.csv')%>%dplyr::select(id, year, everything())%>%
      dplyr::select(-X)%>%dplyr::select(-GEOID_pp, -state_geoid, -msa_geoid, -GeoName)%>%mutate(radius='') %>%table_summary(.)
  }
...
observeEvent(input$pull_gentrification,{
  ...
  if(input$pull_gentrification==0){
  ...
  auto_status(id='all ids', radius='NA', year='2010 & 2019')
  ...
  result <- future({
    ...
    write.csv(dataset_gentrification %>% dplyr::select(id, GEOID10, everything()), '~/workspace/Inspace/data_pull_measures/dataset_gentrification.csv')
...
observeEvent(input$show_data_gentrification, {
  ...
  if(input$show_data_gentrification=='Show measure summary'& file.exists('~/workspace/Inspace/data_pull_measures/dataset_gentrification.csv')==TRUE){
    preview_gentrification$data=read.csv('~/workspace/Inspace/data_pull_measures/dataset_gentrification.csv')%>%dplyr::select(id, everything())%>%
      dplyr::select(-X)%>%dplyr::select(-GEOID10) %>% mutate(year=2019, radius='') %>% table_summary(.)
  }
