...
download_file_places <- function () {
  ...
  if(year==2017){
    download.file(url = places2017url, destfile = "external_data/downloaded_places.csv") 
  }
  if(year==2018){
    download.file(url = places2018url, destfile = "external_data/downloaded_places.csv") 
  }
  if(year==2019){
    download.file(url = places2019url, destfile = "external_data/downloaded_places.csv") 
  }
  }
```
...
### 4. Designate ACMT settings for data pull
...
### 5. Pull estimates of Places data for each lat/long
...
### 6a. 
  ...
  If you only need data from a single year, export your dataset below

```{r export data}

## for only 1 year
setwd("/home/rstudio/workspace/Inspace")
write.csv(datset_places2, 'dataset_places.csv')

```


### 6b. *For multiple years: Rename dataset and rerun with a different year

  For additional years, save this dataset under a different name and re-run from step 1 with the other year of PLACES data. 
  
```{r rename dataset and merge multiple years}
## for multiple years

places2018<-dataset_places2

## return to step 1 and rerun with additional year

```

### 7. *For multiple years: Combine datasets from multiple years and export data

  After pulling Places data estimates for the additional year, combined datasets and export to the Inspace folder

```{r combine multiple years}
places.full<-rbind(datset_places2, dataset)

setwd("/home/rstudio/workspace/Inspace")
write.csv(dataset_places2, 'dataset_places.csv')
