...
latlong_final <- latlong_sf %>%
  ...
  dplyr::select(id, county_GEOID)

dataset_acs_full<-merge(dataset_acs_full, latlong_final)

```

### 6. Additionally, we will pull the metropolitan statistical area (MSA) for each lat/long location

```{r pull MSA}
#download cbsa shapefile
cbsa<-core_based_statistical_areas(year=2019)
#lat-long shapefile with 
latlong_sf<-st_as_sf(dataset_acs, coords=c('long', 'lat'), crs=4269)
cbsa_sf<-st_as_sf(cbsa, crs=4269)
intersected<-st_intersects(latlong_sf, cbsa_sf)

latlong_final <- latlong_sf %>%
  mutate(intersection = as.integer(intersected),
         msa_GEOID = if_else(is.na(intersection), "",
                        as.character(cbsa_sf$GEOID[intersection]))) %>%
  dplyr::select(id, msa_GEOID)

dataset_acs_full<-merge(dataset_acs_full, latlong_final, all=TRUE)

```

### 7. Finally export your dataset
