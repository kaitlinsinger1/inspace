...
### 5. Next, we will pull the metropolitan statistical area (MSA) and county GEOID for each lat/long location

```{r pull MSA and county GEOID}
...
county_dataset<-merge(dataset_acs%>%dplyr::select(id), intersected_county, by='id', all=TRUE)%>%dplyr::select(id, county_geoid=GEOID)

dataset_acs_full<-merge(dataset_acs_full, msa_dataset, by='id', all.x=TRUE)
dataset_acs_full<-merge(dataset_acs_full, county_dataset, by='id', all.x=TRUE)
